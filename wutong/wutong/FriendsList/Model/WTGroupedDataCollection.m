...
#import "WTGroupedDataCollection.h"
...
@interface WTGroupedDataCollection ()
@property (nonatomic, strong) NSMutableOrderedSet *specialGroupTtiles;
...
@implementation WTGroupedDataCollection
...
- (void)setMembers:(NSArray *)members
{
    ...
    for (id<WTGroupMemberProtocol>member in members) {
    ...
    [self.groupTtiles removeAllObjects];
    ...
    [tmp enumerateKeysAndObjectsUsingBlock:^(NSString *groupTitle, NSMutableArray *groupedMembers, BOOL *stop) {
        if (groupTitle.length) {
            ...
            [self.groups addObject:[[Pair alloc] initWithFirst:groupTitle second:groupedMembers]];
        }
    }];
...
{
    [self.groups enumerateObjectsUsingBlock:^(Pair *obj, NSUInteger idx, BOOL *stop) {
        NSMutableArray *groupedMembers = obj.second;
        [groupedMembers sortUsingComparator:^NSComparisonResult(id<WTGroupMemberProtocol> member1, id<WTGroupMemberProtocol> member2) {
